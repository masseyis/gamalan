...
impl TaskPackRepository for SqlTaskPackRepository {
    async fn save_task_pack(&self, task_pack: &TaskPack) -> Result<(), AppError> {
        let non_goals_json = serde_json::to_value(&task_pack.non_goals)
            .map_err(|_| AppError::InternalServerError)?;

        let ac_covered_json = serde_json::to_value(&task_pack.acceptance_criteria_covered)
            .map_err(|_| AppError::InternalServerError)?;

        let constraints_json = serde_json::to_value(&task_pack.constraints)
            .map_err(|_| AppError::InternalServerError)?;

        let test_plan_json = serde_json::to_value(&task_pack.test_plan)
            .map_err(|_| AppError::InternalServerError)?;

        let do_not_list_json = serde_json::to_value(&task_pack.do_not_list)
            .map_err(|_| AppError::InternalServerError)?;

        let commit_plan_json = serde_json::to_value(&task_pack.commit_plan)
            .map_err(|_| AppError::InternalServerError)?;
