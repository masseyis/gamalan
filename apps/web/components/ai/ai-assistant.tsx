...
export function AIAssistant({ projectId, storyId, taskId, context = 'general' }: AIAssistantProps) {
  ...
  const acceptanceCriteriaMutation = useMutation({
    ...
    onSuccess: (data) => {
      const suggestions: AISuggestion[] = data.suggestions.map(
  ...
  const taskClarificationMutation = useMutation({
    ...
    onSuccess: (data) => {
      const suggestions: AISuggestion[] = data.clarifications.map(
        (clarification: any, index: number) => ({
      ...
      setModalDescription(
        'AI-generated clarifications to help understand task requirements better.'
      )
      setModalOpen(true)
  ...
  const sprintPlanningMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      setModalDescription(
        'AI recommendations for your next sprint based on team velocity and story complexity.'
      )
      setModalOpen(true)
    ...
    if (context === 'story' && storyId) {
      baseActions.push(
        ...
        },
        {
          ...
          icon: FileText,
          action: () =>
            executeAction('generate-ac', () => acceptanceCriteriaMutation.mutateAsync()),
        },
        {
          id: 'story-breakdown',
          ...
          icon: Lightbulb,
          action: () =>
            executeAction('story-breakdown', () => storyBreakdownMutation.mutateAsync()),
        },
        {
          id: 'risk-analysis',
          title: 'Analyze Risks',
      ...
      baseActions.push(
        {
          ...
          icon: Play,
          action: () =>
            executeAction('sprint-planning', () => sprintPlanningMutation.mutateAsync()),
        },
        {
          id: 'velocity-prediction',
          ...
          icon: TrendingUp,
          action: () =>
            executeAction('velocity-prediction', () => velocityPredictionMutation.mutateAsync()),
        }
      )
    }
        ...
        <CardContent className="pt-0">
          ...
          <div className="mt-6 p-4 bg-white/10 rounded-xl border border-white/20">
            <div className="flex items-center gap-3 text-sm text-white/80">
              ...
              <span>
                AI features are in beta. Review all suggestions before applying. Your feedback helps
                us improve.
              </span>
            </div>
          </div>
